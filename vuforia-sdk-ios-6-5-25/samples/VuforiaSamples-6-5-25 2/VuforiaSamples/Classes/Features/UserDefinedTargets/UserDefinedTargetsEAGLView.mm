...
- (void)renderFrameWithState:(const Vuforia::State &)state projectMatrix:(Vuforia::Matrix44F &)projectionMatrix
{
    ...
    for (int i = 0; i < state.getNumTrackableResults(); ++i) {
        ...
        if (trackable.getId() == 1) {
            scale = kCoconutScale;
            verts = (GLvoid*)CoconutOBJVerts;
            normals = (GLvoid*)CoconutOBJNormals;
            ...
            rowTexture = 1;
            scale = kTeapotScale;
            verts = (GLvoid*)teapotVertices;
            normals = (GLvoid*)teapotNormals;
            texCoords = (GLvoid*)teapotTexCoords;
        ...
        glBindTexture(GL_TEXTURE_2D, augmentationTexture[rowTexture].textureID);
        glUniformMatrix4fv(mvpMatrixHandle, 1, GL_FALSE, (const GLfloat*)&modelViewProjection.data[0]);
        glUniform1i(texSampler2DHandle, 0 /*GL_TEXTURE0*/);
        if (trackable.getId() == 1) {
            glDrawArrays(GL_TRIANGLES, 0, CoconutOBJNumVerts);
        } else {
